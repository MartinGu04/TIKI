...
function Step1({ state: s, setState, onNext, t }: {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-[11px] font-medium block mb-1.5 text-right" style={{ color: 'var(--t3)' }}>{t.tickerSymbol}</label>
          ...
          {s.results.map((r) => (
            ...
            >
              <div className="text-right min-w-0">
                ...
                <p className="text-[10px] mt-0.5" style={{ color: 'var(--t3)' }}>{r.exchange} · {r.type}</p>
              </div>
              <div className="shrink-0 mr-3">
                <p className="text-sm font-bold ticker" style={{ color: 'var(--t1)' }} dir="ltr">{r.ticker}</p>
                <p className="text-[10px] ticker text-left" style={{ color: 'var(--t3)' }} dir="ltr">{r.symbol}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="rounded-xl p-4" style={{ background: 'var(--a10)', border: '1px solid var(--a20)' }}>
          <div className="flex items-start justify-between">
            <div className="text-right">
              ...
              <p className="text-[10px] mt-0.5" style={{ color: 'var(--t3)' }}>{s.selected.exchange}</p>
            </div>
            <div className="text-left shrink-0 mr-3">
              ...
              {s.priceLoading && (
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-[10px]" style={{ color: 'var(--t3)' }}>{t.fetchingPrice}</span>
                </div>
              )}
          ...
          {s.currentPrice !== null && (
            <div className="flex items-center gap-1 mt-2">
              ...
              <span className="text-[11px]" style={{ color: 'var(--up)' }}>{t.priceReceived}</span>
            </div>
          )}
        </div>
      )}
      ...
      <button onClick={onNext} disabled={!s.selected || s.priceLoading}
...
function Step2({ step1, state: s, setState, onNext, onBack, t }: {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-[11px] font-medium block mb-1.5 text-right" style={{ color: 'var(--t3)' }}>{t.purchaseDate}</label>
      ...
      <div>
        <label className="text-[11px] font-medium block mb-2 text-right" style={{ color: 'var(--t3)' }}>{t.howDidYouInvest}</label>
      ...
      {s.method === 'amount' ? (
        <div>
          <label className="text-[11px] font-medium block mb-1.5 text-right" style={{ color: 'var(--t3)' }}>{t.investedAmount}</label>
          <div className="relative">
        ...
        <div>
          <label className="text-[11px] font-medium block mb-1.5 text-right" style={{ color: 'var(--t3)' }}>{t.unitsPurchased}</label>
...
function Step3({ state: s, setState, step2Method, step2AmountStr, onBack, onSubmit, loading, t }: {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-[11px] font-medium block mb-2 text-right" style={{ color: 'var(--t3)' }}>{t.whoIsOwner}</label>
      ...
      <div>
        <label className="text-[11px] font-medium block mb-1.5 text-right" style={{ color: 'var(--t3)' }}>{t.frequencyLabel}</label>
        ...
        {s.frequency.type === 'monthly' && (
          <div className="mt-2">
            <label className="text-[10px] block mb-1" style={{ color: 'var(--t3)' }}>{t.dayOfMonth}</label>
      ...
      {s.frequency.type !== 'one-time' && step2Method === 'units' && (
        <div>
          <label className="text-[11px] font-medium block mb-1.5 text-right" style={{ color: 'var(--t3)' }}>{t.contributionAmount}</label>
          <div className="relative">
      ...
      <div>
        <label className="text-[11px] font-medium block mb-2 text-right" style={{ color: 'var(--t3)' }}>{t.colorAccent}</label>
...
function EditForm({ asset, onSave, onClose, t }: {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-[11px] font-medium block mb-1.5" style={{ color: 'var(--t3)' }}>{t.name}</label>
        <input type="text" value={form.name} onChange={set('name')} className={INPUT_CLS} />
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="text-[11px] font-medium block mb-1.5" style={{ color: 'var(--t3)' }}>{t.avgBuy}</label>
          <input type="number" min={0} step="any" value={form.avgBuyPrice} onChange={set('avgBuyPrice')} className={INPUT_CLS} dir="ltr" />
        </div>
        <div>
          <label className="text-[11px] font-medium block mb-1.5" style={{ color: 'var(--t3)' }}>{t.qty}</label>
          <input type="number" min={0} step="any" value={form.quantity} onChange={set('quantity')} className={INPUT_CLS} dir="ltr" />
        </div>
        <div>
          <label className="text-[11px] font-medium block mb-1.5" style={{ color: 'var(--t3)' }}>{t.currentPrice}</label>
          <input type="number" min={0} step="any" value={form.currentPrice} onChange={set('currentPrice')} className={INPUT_CLS} dir="ltr" />
        </div>
      </div>
      <div>
        <label className="text-[11px] font-medium block mb-2" style={{ color: 'var(--t3)' }}>{t.whoIsOwner}</label>
        <div className="grid grid-cols-3 gap-2">
          {ownerOpts.map((o) => (
      ...
      <div>
        <label className="text-[11px] font-medium block mb-1.5" style={{ color: 'var(--t3)' }}>{t.frequencyLabel}</label>
      ...
      {form.frequency.type !== 'one-time' && (
        <div>
          <label className="text-[11px] font-medium block mb-1.5" style={{ color: 'var(--t3)' }}>{t.contributionAmount}</label>
          <input type="number" min={0} step="any" value={form.contributionStr} onChange={set('contributionStr')} className={INPUT_CLS} dir="ltr" />
        </div>
      )}
      <div>
        <label className="text-[11px] font-medium block mb-2" style={{ color: 'var(--t3)' }}>{t.colorAccent}</label>
        <div className="flex flex-wrap gap-2">
          {PRESET_COLORS.map((color) => (
...
export function AssetModal({ mode, asset, onSave, onClose }: Props) {
  ...
  const handleSubmit = () => {
    ...
    const newAsset: Asset = {
      ...
      monthlyContribution: s3.frequency.type !== 'one-time'
        ? (s2.method === 'amount' ? parseFloat(s2.amountStr) || 0 : parseFloat(s3.contributionStr) || 0)
        : 0,
      frequency: s3.frequency, color: s3.color, lastPriceUpdate: Date.now(),
    };
    setSubmitting(false);
    onSave(newAsset);
  };
              ...
              <div className="flex items-center gap-2 mt-1.5">
                {[1, 2, 3].map((n) => (
                  <div key={n} className="flex items-center gap-1">
                    ...
                    <span className="text-[10px]" style={{ color: step === n ? 'var(--t2)' : 'var(--t4)' }}>
                      {stepLabels[n - 1]}
                    </span>
